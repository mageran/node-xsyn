...
GrammarDef.prototype.__defineGetter__('userDefinedStartNonterminal', function() {
  if (typeof(this.$userDefinedStartNonterminal$) === 'undefined') {
    this.$userDefinedStartNonterminal$ = null;
  }
...
GrammarDef.prototype.__defineGetter__('customTokens', function() {
  if (typeof(this.$customTokens$) === 'undefined') {
    this.$customTokens$ = {};
  }
  return this.$customTokens$;
});
GrammarDef.prototype.__defineSetter__('customTokens', function(customTokens) {
...
/**
 * @method addCustomTokenDefinition(tokenName,regExp,action)
 * @returns void
 */
GrammarDef.prototype.addCustomTokenDefinition = function(tokenName,regExp,action) {
  var internalTokenName = '__token__' + tokenName;
  var ntname = tokenName;
  this.addProductionRule(ntname,internalTokenName,action);
  var regexp = new RegExp(regExp);
  this.customTokens[internalTokenName] = regexp;
}
...
/**
 * @method setStartNonterminal(name,dontCreateNew,setByUser)
 * @returns xsyn.grammar.INonterminal
 */
GrammarDef.prototype.setStartNonterminal = function(name,dontCreateNew,setByUser) {
  var nt = this.getNonterminal(name);
  if (nt === null) {
    throw '*** no such nonterminal: \'' + name + '\'';
  }
  if (setByUser) {
    this.userDefinedStartNonterminal = name;
  }
  if (dontCreateNew) {
    this.startNonterminal = nt;
    return nt;
...
GrammarDef.prototype.compile = function(input,options) {
  ...
  if (opts.mode === 'asModule') {
    ...
    headerCode += ' */\n';
    headerCode += 'var xsyn = require(\'node-xsyn\');\n';
  }
  var body = '';
...
GrammarDef.prototype.requireAsModule = function(input,moduleContext) {
  ...
  var filename = mname + '.js';
  if (false) try {
    var fs = require('fs');
    fs.writeFileSync(filename,src,'utf-8');
  } catch(e) {
    throw ('problems writing tmp module file:' + e);
  }
  //return moduleContext.require(mname);
  var Module = moduleContext.constructor;
  var m = new Module(mname,moduleContext);
